            ...
            try:
                for iteration in range(iterations):
                    ...
                    for episode in tqdm(range(num_episodes)):
                        ...
                        sharpe = analyser.computeSharpeRatio()
                        episode_sharpes.append(sharpe)
                        
                        # Log metrics
                        if self.writer is not None:
                            self.writer.add_scalar('Training/episode_reward', episode_reward, episode)
                            self.writer.add_scalar('Training/episode_length', steps, episode)
                            self.writer.add_scalar('Training/sharpe_ratio', sharpe, episode)
                        ...
                        if episode % 5 == 0:
                            ...
                            if self.writer is not None:
                                self.writer.add_scalar('Testing/sharpe_ratio', test_sharpe, episode)
